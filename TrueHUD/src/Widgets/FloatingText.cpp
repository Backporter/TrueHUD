...
namespace Scaleform
{
	...
	void FloatingText::UpdatePosition(float a_deltaTime)
	{
		...
		if (_bIs3D) {
			...
			ConsoleRE::NiCamera::WorldPtToScreenPt3((float(*)[4])g_worldToCamMatrix, *g_viewPort, _worldPosition, _screenPosition.x, _screenPosition.y, _depth, 1e-5f);

			bool bIsOffscreen = _depth < 0;

			ConsoleRE::GFxValue visible;
