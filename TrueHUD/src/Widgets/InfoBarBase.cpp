...
namespace Scaleform
{
	...
	void InfoBarBase::FlashActorValue(ConsoleRE::ActorValue a_actorValue, bool a_bLong)
	{
	...
	void InfoBarBase::RefreshColors()
	{
		...
		args[0].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kHealth, ColorType::BarColor));
		args[1].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kHealth, ColorType::PhantomColor));
		args[2].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kHealth, ColorType::BackgroundColor));
		args[3].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kHealth, ColorType::PenaltyColor));
		args[4].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kHealth, ColorType::FlashColor));
		args[5].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kMagicka, ColorType::BarColor));
		args[6].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kMagicka, ColorType::PhantomColor));
		args[7].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kMagicka, ColorType::BackgroundColor));
		args[8].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kMagicka, ColorType::PenaltyColor));
		args[9].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kMagicka, ColorType::FlashColor));
		args[10].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kStamina, ColorType::BarColor));
		args[11].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kStamina, ColorType::PhantomColor));
		args[12].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kStamina, ColorType::BackgroundColor));
		args[13].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kStamina, ColorType::PenaltyColor));
		args[14].SetNumber(hudMenu->GetBarColor(_refHandle, ConsoleRE::ActorValue::kStamina, ColorType::FlashColor));
	...
	{	
		...
		if (_widgetState < kDelayedRemoval) {
			...
			if (targetState > kBleedingOut) {
				...
				if (_bIsOffscreen) {
					...
					bool bVisible = bHasLOS && !(actor->AsActorValueOwner()->GetActorValue(ConsoleRE::ActorValue::kInvisibility) > 0);
					if (bVisible) {
						SetWidgetState(WidgetStateMode::kShow);
					} else {
						SetWidgetState(WidgetStateMode::kHide);
		...
		float permanentHealth = actorValueOwner->GetPermanentActorValue(ConsoleRE::ActorValue::kHealth);
		float temporaryHealth = actor->GetActorValueModifier(ConsoleRE::ACTOR_VALUE_MODIFIER::kTemporary, ConsoleRE::ActorValue::kHealth);
		...
		float permanentMagicka = actorValueOwner->GetPermanentActorValue(ConsoleRE::ActorValue::kMagicka);
		float temporaryMagicka = actor->GetActorValueModifier(ConsoleRE::ACTOR_VALUE_MODIFIER::kTemporary, ConsoleRE::ActorValue::kMagicka);
		float maxMagicka = permanentMagicka + temporaryMagicka;
		float magicka = actorValueOwner->GetActorValue(ConsoleRE::ActorValue::kMagicka);
		float permanentStamina = actorValueOwner->GetPermanentActorValue(ConsoleRE::ActorValue::kStamina);
		float temporaryStamina = actor->GetActorValueModifier(ConsoleRE::ACTOR_VALUE_MODIFIER::kTemporary, ConsoleRE::ActorValue::kStamina);
		float maxStamina = permanentStamina + temporaryStamina;
		float stamina = actorValueOwner->GetActorValue(ConsoleRE::ActorValue::kStamina);
		
		ConsoleRE::GFxValue args[18];
