...
bool Utils::GetNodePosition(ConsoleRE::ActorPtr a_actor, const char* a_nodeName, ConsoleRE::NiPoint3& point)
{
	bool bResult = false;
...
bool Utils::GetTorsoPos(ConsoleRE::ActorPtr a_actor, ConsoleRE::NiPoint3& point)
{
	...
	ConsoleRE::BGSBodyPart* bodyPart = bodyPartData->parts[0];
	if (!bodyPart)
	{
		return false;
	}
...
bool Utils::GetTargetPos(ConsoleRE::ObjectRefHandle a_target, ConsoleRE::NiPoint3& pos, bool bGetTorsoPos /*= true*/)
{
	...
	if (target->formType == static_cast<uint32_t>(ConsoleRE::FormType::ActorCharacter)) 
	{
		auto actorPtr = ConsoleRE::ActorPtr(target->As<ConsoleRE::Actor>());
...
bool Utils::IsPlayerTeammateOrSummon(ConsoleRE::Actor* a_actor)
{
	if (a_actor) {
...
bool Utils::IsBehindPlayerCamera(const ConsoleRE::NiPoint3& a_pos)
{
	...
	ConsoleRE::NiQuaternion cameraRotation{ 1.f, 0.f, 0.f, 0.f };
...
ConsoleRE::NiQuaternion Utils::QuatFromRotationMatrix(const ConsoleRE::NiMatrix3& a_matrix)
{
	ConsoleRE::NiQuaternion ret;
...
ConsoleRE::NiMatrix3 Utils::QuaternionToMatrix(const ConsoleRE::NiQuaternion& a_quat)
{
...
Utils::Matrix4::Matrix4(const ConsoleRE::NiPoint3& a_x, const ConsoleRE::NiPoint3& a_y, const ConsoleRE::NiPoint3& a_z, const ConsoleRE::NiPoint3& a_w)
{
	entry[0][0] = a_x.x;
	entry[0][1] = a_x.y;
	entry[0][2] = a_x.z;
...
void Utils::Matrix4::SetAxis(const ConsoleRE::NiPoint3& a_axis0, const ConsoleRE::NiPoint3& a_axis1, const ConsoleRE::NiPoint3& a_axis2, const ConsoleRE::NiPoint3& a_origin)
{
	entry[0][0] = a_axis0.x;
	entry[0][1] = a_axis0.y;
	entry[0][2] = a_axis0.z;
...
ConsoleRE::NiPoint3 Utils::Matrix4::TransformVector4(const ConsoleRE::NiPoint3& a_vector, float a_w) const
{
...
ConsoleRE::NiPoint3 Utils::Matrix4::TransformPosition(const ConsoleRE::NiPoint3& a_position) const
{
	return TransformVector4(a_position, 1.f);
}

ConsoleRE::NiPoint3 Utils::Matrix4::TransformVector(const ConsoleRE::NiPoint3& a_vector) const
{
...
Utils::MatrixScale::MatrixScale(const ConsoleRE::NiPoint3& a_scale)
{
	entry[0][0] = a_scale.x;
	entry[0][1] = 0.f;
	entry[0][2] = 0.f;
...
Utils::MatrixQuatRotation::MatrixQuatRotation(const ConsoleRE::NiQuaternion& a_quat)
{
